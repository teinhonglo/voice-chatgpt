...
Stages:
  ...
  1  Evaluate direct prompt transfer on GPT-Realtime
  2  Optimize a Full-Duplex prompt with GEPA
  3  Re-evaluate the optimized prompt on held-out test scenarios
...
if (( stage <= 0 && stop_stage >= 0 )); then
  python experiments/auto_prompt/experiment.py prepare     "${common[@]}"     "${prompt_arg[@]}"     --scenarios "${scenarios}"     --references "${exp_dir}/references.jsonl"
fi
...
PY
  ...
  python experiments/auto_prompt/experiment.py evaluate     "${common[@]}"     --prompt "${baseline_prompt}"     --references "${exp_dir}/references.jsonl"     --split test     --judge-repeats 3     --output "${exp_dir}/direct_transfer_test.jsonl"
fi

if (( stage <= 2 && stop_stage >= 2 )); then
  python experiments/auto_prompt/experiment.py optimize     "${common[@]}"     "${prompt_arg[@]}"     --references "${exp_dir}/references.jsonl"     --max-metric-calls "${max_metric_calls}"     --output-dir "${exp_dir}/gepa"
fi

if (( stage <= 3 && stop_stage >= 3 )); then
  python experiments/auto_prompt/experiment.py evaluate     "${common[@]}"     --prompt "${exp_dir}/gepa/best_system_prompt.txt"     --references "${exp_dir}/references.jsonl"     --split test     --judge-repeats 3     --output "${exp_dir}/gepa/final_test.jsonl"
fi
