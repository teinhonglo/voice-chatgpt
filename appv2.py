...
output_wav_path = "demos/tts.mp3"
warmup_reply = WARMUP_REPLY
grade_reply = GRADE_REPLY
...
class TTS:
    ...
    def synthesis(self, text: Optional[str] = "") -> str:
...
def setup_prompt(chat_history, user_id, init_prompt, llm, tts_spk):
    if init_prompt == "":
        init_prompt = INIT_PROMPT
    
    chat_bots[user_id] = Chat(init_prompt=init_prompt, llm=llm)
    tts_bots[user_id] = TTS(voice=tts_spk)
    ...
    output_wav_path = tts.synthesis(bot_message)
...
with gr.Blocks(css=CSS) as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            tts_spk = gr.Dropdown(["alloy", "echo", "fable", "onyx", "nova", "shimmer"], value="alloy", label="TTS speaker")
    ...
    setup_button.click(setup_prompt, [chatbot, user_id, init_prompt, llm, tts_spk], [chatbot, syn_audio])
